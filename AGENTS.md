...
- **教学模式默认"只教不改"**：当用户以"教我做 / 教我实现 / 逐步指导"等学习意图提出需求时，助手**不得直接创建、修改或删除用户项目中的任何文件**，除非用户明确要求"你来改 / 帮我直接实现"等授权。正确做法是：给出清晰的步骤做法、可直接粘贴的完整代码与代码逐段讲解，由用户自己动手落地；验证类命令也只提供让用户自己执行。该默认只对教学意图生效，普通的"帮我实现一个功能"开发请求不受此限制，仍按 §7 正常直接改代码。
