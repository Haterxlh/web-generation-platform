...
### 5.2 `prompt` 的契约：必须独立自洽（**2026-09-15 实测定的**）

`POST /api/generation/create` 收到的 `prompt` 有**两个**读者，且它们的可见范围不同：

| 读者 | 位置 | 能看到什么 |
|---|---|---|
| 意图路由（API 侧） | `AgentChatService.chat` → `intent_router.route()` | 与 chat-agent **同一份** history + 本轮附件 + 合并前的草稿槽位 |
| ROUTING（worker 侧） | `orchestrator.run` ⑧ | **只有 `prompt`** + 会话草稿（阶段 8 起）+ 附件 digest；**没有对话历史** |

因此调用方（前端 / 第三方）必须遵守：

1. **`prompt` 里只写需求本身**，不得夹带闲聊、开场白或"上下文"类内容 ——
   worker 的 ROUTING 会把它当作**用户需求原文**参与完备度判定；
2. 前端会话式流程的做法可作参考：只把**用户已确认的槽位**拼成完整句子
   （"请生成一个…页面。核心功能：…。视觉风格：…。"），与需求确认卡片显示的内容同源；
3. 用户原话只在"槽位拼不出东西"时兜底，且取**最后一条**（第一条常常是"你能做什么"这类开场白）。

> **为什么这条契约必须写下来**：2026-09-15 实测踩到 —— 前端把第一条用户消息附在 prompt 末尾当"原始描述"，
> 而那条消息正是"你能做什么？"，于是 worker 的 ROUTING 把整段读成能力咨询，任务停在 `clarifying`，
> 用户刚对着需求确认卡片点过"开始生成"却被说"信息不够"。
> 之后又验证过：**把会话草稿接上 worker 也不能单独挡住这种污染**（草稿让 need_rag / merge 受益，
> 但 router 就是被设计来识别"用户在问能力"的）—— 所以这条契约只能由调用方承担。
...
| 2026-09-15 | 新增 **§5.2 `prompt` 的契约：必须独立自洽** —— 由"前端把开场白附进 prompt 导致 worker ROUTING 误判"这次实测事故沉淀而来 |
